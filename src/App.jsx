...
/* ── storage: window.storage when present, localStorage otherwise ─ */
const hasStore =
...
async function sset(key, val) {
  ...
  if (hasStore) {
    try { await window.storage.set(key, v); } catch {}
  } else {
...
/* ── pixel notebook icon (blue, for saved day note) ── */
function PixelNotebookIcon({ size = 30 }) {
  const cover = "#A8BCDC";   // lighter blue cover
  const coverHi = "#C4D4EC";
  const coverLo = "#8CA0C8";
  const page = "#F2F5FA";
  const line = "#C8D2E2";
  const dk = "#9DAAB5";      // lighter grey border
  const rects = [];
  const p = (x, y, c) => rects.push({ x, y, c });
  // 16x16 grid
  // outer dark border of cover
  for (let x = 3; x <= 14; x++) { p(x, 1, dk); p(x, 15, dk); }
  for (let y = 2; y <= 14; y++) { p(2, y, dk); p(14, y, dk); }
  // spine (left binding)
  for (let y = 2; y <= 14; y++) { p(3, y, coverLo); p(4, y, coverLo); }
  // cover fill
  for (let y = 2; y <= 14; y++)
    for (let x = 5; x <= 13; x++)
      p(x, y, cover);
  // cover highlight top
  for (let x = 5; x <= 13; x++) p(x, 2, coverHi);
  // page area (inset right portion)
  for (let y = 3; y <= 13; y++)
    for (let x = 6; x <= 12; x++)
      p(x, y, page);
  // ruled lines on page
  for (let x = 7; x <= 11; x++) { p(x, 5, line); p(x, 8, line); p(x, 11, line); }
  // binding rings
  p(4, 4, dk); p(4, 8, dk); p(4, 12, dk);
  return (
    <svg width={size} height={size} viewBox="0 0 16 17" shapeRendering="crispEdges" aria-hidden="true" style={{ display: "block", flexShrink: 0 }}>
      {rects.map((r, i) => <rect key={i} x={r.x} y={r.y} width={1} height={1} fill={r.c} />)}
    </svg>
  );
}
...
export default function App() {
  ...
  async function runImport(file) {
    ...
    try {
      ...
      for (const [key, val] of Object.entries(data.days)) {
        ...
        let idx = indexRef.current;
        if (!idx) idx = (await sget("index:v2")) || { v: 1, days: {} };
        idx.days[ds] = summarize(val);
        indexRef.current = idx;
        await sset("index:v2", idx);
        count++;
      }
      // reload current day
      const d = await sget("day:" + activeDate);
      if (d) setDay(d);
  ...
  return (
    <div style={S.root}>
      ...
      <div style={S.col}>
        <header style={S.bar}>
          ...
          <nav style={{ display: "flex", gap: 16, justifyContent: "center" }}>
            ...
            <button style={S.navlink(false)} onClick={() => setImportOpen(true)}>import</button>
          </nav>
        </header>
      ...
      {importOpen && (
        <div style={styles.scrim} onClick={() => { setImportOpen(false); setImportStatus(""); }}>
          <div style={styles.sheet} onClick={(e) => e.stopPropagation()}>
            <div style={styles.handle} />
            <div style={styles.sheetTitle}>import data</div>
            <p style={styles.exportHint}>Pick a spoons JSON export file to restore your data.</p>
            {importStatus ? (
              <p style={{ fontSize: 14, color: C.ink, textAlign: "center", padding: "20px 0" }}>{importStatus}</p>
            ) : (
              <label style={{
                display: "flex", alignItems: "center", justifyContent: "center",
                width: "100%", minHeight: 58, background: C.surface,
                border: `1px solid #7A8A96`, cursor: "pointer", fontSize: 14,
                color: C.ink, boxShadow: `2px 2px 0 #C8D2D8`,
              }}>
                choose file
