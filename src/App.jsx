...
/* ── storage: on-device localStorage first; window.storage if this runs
   inside Claude; in-memory as a last resort. All guarded so a blocked store
   never throws — the UI just falls back. ─────────────────────────────── */
const mem = new Map();
const hasClaude =
...
const styles = {
  ...
  col: { width: "100%", maxWidth: 480, height: "100%", display: "flex", flexDirection: "column", overflow: "hidden" },
  bar: {
    flex: "none",
    display: "flex",
  ...
  navArrow: { fontSize: 22, color: C.inkSoft, minWidth: 44, minHeight: 44 },
  dateLabel: {
    fontSize: 13,
    letterSpacing: 1,
    textTransform: "uppercase",
  ...
  remove: { fontSize: 18, color: C.inkFaint, minWidth: 44, minHeight: 44 },
  entryTime: {
    color: C.inkSoft,
    minWidth: 48,
    textAlign: "left",
